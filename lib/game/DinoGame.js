...
export default class DinoGame extends GameRunner {
  constructor(width, height) {
    ...
    this.defaultSettings = {
      bgSpeed: 8, // ppf
      birdSpeed: 7.0, // ppf
      birdSpawnRate: 120, // fpa (spawn birds twice as often)
      birdWingsRate: 10, // fpa
      birdGroundClearance: 10, // px clearance above ground/ducking dino
      cactiSpawnRate: 50, // fpa
      cloudSpawnRate: 200, // fpa
      cloudSpeed: 2, // ppf
      dinoGravity: 0.5, // ppf
      dinoGroundOffset: 5, // px
      dinoLegsRate: 6, // fpa
      dinoLift: 10, // ppf
  ...
  drawCacti() {
    ...
    if (this.frameCount % settings.cactiSpawnRate === 0) {
      ...
      if (!state.birds.length && randBoolean() &&
          this.frameCount - state.lastObstacleFrame >= minObstacleGap) {
        ...
        cacti.push(newCacti)
        state.lastObstacleFrame = this.frameCount
      }
    }
  ...
  drawBirds() {
    ...
    if (this.frameCount % settings.birdSpawnRate === 0) {
      ...
      if (randBoolean() &&
          this.frameCount - this.state.lastObstacleFrame >= minObstacleGap) {
        ...
        // spawn closer to ground (adjustable via birdGroundClearance)
        newBird.y =
          this.height -
          Bird.maxBirdHeight -
          Bird.wingSpriteYShift -
          sprites.dinoDuckLeftLeg.h / 2 +
          settings.birdGroundClearance -
          settings.dinoGroundOffset
        birds.push(newBird)
        this.state.lastObstacleFrame = this.frameCount
      }
    }
